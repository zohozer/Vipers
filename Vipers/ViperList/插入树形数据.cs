...
namespace Vipers
{
    public class InsertTree : GH_Component
    {
        /// <summary>
        /// Initializes a new instance of the 列表插入树形数据 class.
        /// </summary>
        public InsertTree()///////TangChi 2015.3.18
            : base("插入树形数据", "InsertTree",
                "将子树形数据的每个分支按照索引插入源树形数据",
               "Vipers", "Viper.data")
        {
        }
        ...
        {
            pManager.AddGenericParameter("源树形数据", "T1", "该树形数据将被子树形数据插入", GH_ParamAccess.tree);
            pManager.AddGenericParameter("子树形数据", "T2", "该树形数据将插入源树形数据", GH_ParamAccess.tree);
            pManager.AddIntegerParameter("索引值", "I", "子树形数据的每个列表将按索引值分别插入源树形数据", GH_ParamAccess.list);
        ...
        {
            get { return new Guid("{35511b59-ed18-4755-a8c5-ae86b610b01c}"); }
        }
    }
}